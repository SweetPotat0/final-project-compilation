...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_cond_ribs ribs =
    ...
    | ScmPair (ScmPair (ScmSymbol "else", exprs), ribs) ->
         (match ribs with
            | ScmNil -> ScmPair (ScmSymbol "begin", exprs)
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
      ...
      | (ribs', ScmNil) -> tag_parse (ScmPair(ScmPair (ScmSymbol "lambda", ScmPair(scheme_sexpr_list_of_sexpr_list(List.map (fun (v) -> (match v with | ScmPair(c,_) -> c | _ -> raise (X_this_should_not_happen "ribs invalid in let expression [1]"))) ribs'),exprs)),
                                       (scheme_sexpr_list_of_sexpr_list(List.map (fun (expr)-> (match expr with | ScmPair(_,ScmPair(c,ScmNil)) -> c | _ -> raise (X_this_should_not_happen "ribs invalid in let expression [2]"))) ribs'))))
      | _ -> raise (X_syntax "Improper sequence"))
    ...
    | ScmPair (ScmSymbol "let*",
               ScmPair (ScmPair (ScmPair (var,
                                 ...
                                 ribs),
                        exprs)) ->
                           tag_parse (ScmPair (ScmSymbol "let", ScmPair (ScmPair (ScmPair (var,
                                          ScmPair (arg, ScmNil)),
                                 ScmNil),
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
      ...
      | (ribs_list,ScmNil) -> 
         ...
         | (exprs_list, ScmNil) -> 
       tag_parse (ScmPair(ScmSymbol "let", ScmPair(                                          
         scheme_sexpr_list_of_sexpr_list(List.map (fun (v) -> (match v with | ScmPair(c,_) -> ScmPair(c, ScmPair(ScmPair(ScmSymbol "quote", ScmPair (ScmSymbol "whatever", ScmNil)),ScmNil))| _ -> raise (X_this_should_not_happen "ribs invalid in letrec"))) ribs_list),
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmVarGet (Var str) ->
         (match (tag_lexical_address_for_var str params env) with
         | Var'(_,lexadd) -> ScmVarGet' (Var'(str, lexadd)))          
      ...
      | ScmVarSet(Var v, expr) -> (match (tag_lexical_address_for_var v params env) with
         | Var'(_,lexadd) -> ScmVarSet' (Var'(v, lexadd), run expr params env))         
      (* this code does not [yet?] support nested define-expressions *)
      | ScmVarDef(Var v, expr) -> ScmVarSet' (Var'(v, Free), run expr params env)
  ...
  let find_reads_and_writes =
    let rec run name expr params env =
      ...
      | ScmLambda' (params', Simple, expr) ->
         ...
         then ([], [])
         else run name expr params' (params :: env)
      | ScmLambda' (params', Opt opt, expr) ->
  ...
  let cross_product as' bs' =
   ...
   let rec list_any pred = function
      | [] -> false
      | first :: rest -> (pred first) || (list_any pred rest);;

   let rec list_all pred = function
      | [] -> true
      | first :: rest -> (pred first) && (list_all pred rest);;

  let should_box_var name expr params = match find_reads_and_writes name expr params with
      | ([],_) -> false
      | (_, []) -> false
      | (reads,writes) -> 
            list_any (fun ((_,readEnv),(_,writeEnv)) ->
               (list_all (fun (params1,params2) -> 
                  (params1 == params2 && ((is_member name params1) = false) && ((is_member name params2) = false)) = false) (cross_product readEnv writeEnv)
               )) (cross_product reads writes)
   ;;
...
let show str = Semantic_Analysis.semantics (Tag_Parser.tag_parse (Reader.nt_sexpr str 0).found);;
